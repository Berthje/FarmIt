...
export const LoadingScreen = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <div className="relative min-h-screen">
            <div
                className={`transition-opacity duration-1000 ${showLoading ? "opacity-0" : "opacity-100"}`}
            >
                {children}
            </div>
            ...
            {showLoading && (
                <div
                    className={`fixed inset-0 bg-gradient-to-b from-green-600 to-green-800 flex flex-col items-center justify-center transition-opacity duration-1000 ${fadeOut ? "opacity-0" : "opacity-100"}`}
                >
